...
class BinaryTree(Generic[T]):
    ...
    def _is_balanced_helper(self, node: BinaryNode[T]) -> bool:
        if node is None:
            return True
        return (
            abs(self._height_helper(node.left) - self._height_helper(node.right)) <= 1
            and self._is_balanced_helper(node.left)
            and self._is_balanced_helper(node.right)
        )
    ...
    def _is_bst_helper(self, node: BinaryNode[T], min: T, max: T) -> bool:
        ...
        if node.data <= min or node.data >= max:
            return False
        return self._is_bst_helper(node.left, min, node.data) and self._is_bst_helper(
